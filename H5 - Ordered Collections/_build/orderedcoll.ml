...
module BinSTree (Elt : COMPARABLE)
  ...
  struct
    ...
    let rec search (x : elt) (t : tree) : bool =
      ...
      | Branch (left, lst, right) ->
          match lst with
          | [] -> failwith "search: empty list as node"
          | h :: _ ->
            match Elt.compare x h with
    ...
    let rec getmin (t : tree) : elt =
      ...
      | Branch (Leaf, lst, _) ->
          ...
          | [] -> failwith "getmin: empty list as node"
          | h :: _ -> h)
    ...
    let rec getmax (t : tree) : elt =
      ...
      | Branch (_, lst, Leaf) ->
          (match List.rev lst with
          | [] -> failwith "getmax: empty list as node"
          | h :: _ -> h)
...
let reflection () : string =
  "I ran into a lot of trouble trying to break down fix. I tried writing it"
  ^ "out on paper and my thought process was a lot clearer. In the future"
  ^ "I will not underestimate the power of doing out code on paper.";;
