...
let rec to_run_length (lst : char list) : (int * char) list =
  ...
  | [] -> []
  | c1 :: ct ->
      match to_run_length ct with
      | [] -> [(1, c1)]
      | (l, c2) :: tail ->
          if c1 <> c2 then (1, c1) :: (l, c2) :: tail
          else (l + 1, c2) :: tail ;;

let rec from_run_length (lst : (int * char) list) : char list =
  match lst with
  | [] -> []
  | (0, _) :: tail -> from_run_length tail
  | (l, c) :: tail -> c :: from_run_length ((l - 1, c) :: tail) ;;
...
let rec extract_entry (desired_play: play) (matrix : payoff_matrix) : int * int =
  match matrix with
  | [] -> (404, 404)
  | (matrix_play, payoff) :: tail ->
      if desired_play = matrix_play then payoff
      else extract_entry desired_play tail ;;
...
(* Counts instances of a given action for each player given a history *)
let rec count_actions (hist : history) (act : action) : int * int =
  match hist with
  | [] -> (0, 0)
  | (a1, a2) :: tail ->
    let (t1, t2) = count_actions tail act in
    ((if a1 = act then t1 + 1 else t1),
     (if a2 = act then t2 + 1 else t2)) ;;

let count_defections (hist : history) : int * int =
  count_actions hist cDEFECT ;;

let count_cooperations (hist : history) : int * int =
  count_actions hist cCOOPERATE ;;
...
let egalitarian (hist : history) : action =
  ...
  | [] -> cCOOPERATE
  | _ :: _ ->
    let (c1, c2) = count_defections hist in
    if c1 < c2 then cDEFECT else cCOOPERATE ;;
...
let tit_for_tat (hist : history) : action =
  match hist with
  | [] -> cCOOPERATE
  | (_, a) :: _ -> a ;;
...
(* This strategy cooperates on first round, mimics the other player on the
   second round and cooperates only if the other player's past two actions
   were cooperations on all subsequent rounds *)

let my_strategy (hist : history) : action =
  match hist with
  | [] -> cCOOPERATE
  | [(_, a)] -> a
  | (_, a1) :: (_, a2) :: _ ->
      if a1 = cCOOPERATE && a2 = cCOOPERATE then cCOOPERATE
      else cDEFECT ;;
...
let rec swap_actions (hist : history) : history =
  match hist with
  | [] -> []
  | (a1, a2) :: tail -> (a2, a1) :: swap_actions tail ;;
...
let rec calculate_payoff (matrix : payoff_matrix) (hist : history) : int * int =
  match hist with
  | [] -> (0, 0)
  | a :: tail ->
      let (t1, t2)= calculate_payoff matrix tail in
      let (p1, p2) = extract_entry a matrix in
      (t1 + p1, t2 + p2);;
...
let minutes_spent_on_pset () : int = 500 ;;
...
let reflection () : string = ("I ran into the most problems with run length encoding,"
  ^ "especially to_run_length. It was really hard to wrap my head around the recursive"
  ^ "method of writing the function as I'm used to approaching programming problems"
  ^ "from an imperative programming standpoint. By focusing on exactly what I was going"
  ^ "to make the recursive call on inside my function, I was able to piece together my"
  ^ "function. One strategy I could have adopted which might reduced the time needed to"
  ^ "generate my submission was to create more auxiliary functions to assist my testing,"
  ^ "as I ended up repeating a lot of my code within my tests.") ;;
