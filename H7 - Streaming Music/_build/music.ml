...
let rec list_to_stream (lst : obj list) : event NLS.stream =
  let rec list_to_stream_aux remaining =
    ...
    | Note (pitch, duration, volume) :: tail ->
      lazy (NLS.Cons (Tone (0., pitch, volume),
                  lazy (NLS.Cons (Stop (duration, pitch),
                              list_to_stream_aux tail))))
    | Rest (duration) :: tail ->
        shift_start duration (list_to_stream_aux tail)
    | [] -> list_to_stream lst
...
let rec pair (a : event NLS.stream) (b : event NLS.stream)
             : event NLS.stream =
  let head_a = NLS.head a in
  let head_b = NLS.head b in
  let time_head_a = time_of_event (head_a) in
  let time_head_b = time_of_event (head_b) in
  if time_head_a <= time_head_b then
    lazy (NLS.Cons (head_a, pair (NLS.tail a) (shift_start (~-.time_head_a) b)))
  else
    lazy (NLS.Cons (head_b, pair (shift_start (~-.time_head_b) a) (NLS.tail b))) ;;
...
let transpose (str : event NLS.stream) (half_steps : int)
            : event NLS.stream =
  let transpose_aux (e : event) : event =
    match e with
    | Tone (time, pitch, volume) ->
        Tone (time, transpose_pitch pitch half_steps, volume)
    | Stop (time, pitch) ->
        Stop (time, transpose_pitch pitch half_steps) in
  NLS.smap transpose_aux str ;;
...
let canon = pair (shift_start 6.0 melody)
                 (pair (shift_start 4.0 melody)
                       (pair (shift_start 2.0 melody) bass));;
...
let part5 = list_to_stream
              [Note((G, 4), 0.125, 60);
               Note((A, 4), 0.125, 60); Note((Bb, 4), 0.125, 60);
               Note((C, 5), 0.125, 60); Note((A, 4), 0.25, 60);
               Note((F, 4), 0.125, 60); Note((G, 4), 0.375, 60);
               Rest(0.75)];;

output_midi "part1.mid" (stream_to_hex 12 part1);;
output_midi "part2.mid" (stream_to_hex 14 part2);;
output_midi "part3.mid" (stream_to_hex 12 part3);;
output_midi "part4.mid" (stream_to_hex 18 part4);;
output_midi "part5.mid" (stream_to_hex 14 part5);;
