...
let aitken (s: float stream) : float stream =
  let stream1 = smap2 (fun x y -> (x -. y) ** 2.) s (tail s) in
  let stream2 = smap2
                  (fun x y -> x +. y)
                  (smap2 (fun x y -> x -. 2. *. y) s (tail s))
                  (tail (tail s)) in
  let stream3 = smap2 (fun x y -> x /. y) stream1 stream2 in
  smap2 (fun x y -> x -. y) s stream3 ;;
...
let print_depth (n : int) (indent : int) (t : 'a tree) =
  let rec print_spaces (n : int) : unit = 
    if n > 0 then (Printf.printf " "; print_spaces (n - 1))
    else () in
  let rec traverse_tree (tlst : 'a tree list) (level : int) : unit =
    if level = n then ()
    else
      match tlst with
      | hd :: tl ->
        print_spaces (indent + level + 1); 
        Printf.printf "%i\n" (node hd);
        traverse_tree (children hd) (level + 1);
        traverse_tree tl level;
      | [] -> () in
  print_spaces indent; 
  Printf.printf "%i\n" (node t); 
  traverse_tree (children t) 0 ;;
...
let rec bfenumerate (tree_list : 'a tree list) : 'a stream =
    ...
    | hd :: tl -> 
      let Node(a, rest) = Lazy.force hd in 
      lazy (Cons (a, bfenumerate (tl @ rest)))
    | [] -> raise (Finite_tree) ;;
...
let rec levels (n : int) : int tree =
  lazy (Node (n, [levels (succ n); levels (succ n)])) ;;
...
let tree_nats : int tree =
  let rec iterate (current : int): int tree =
    lazy (Node (current + 1, [iterate ((current * 2) + 1); iterate ((current * 2) + 2)])) in 
  iterate 0 ;;
